...
**Vista pública** — cualquiera con el enlace ve dos pestañas de solo lectura:
Supervisores y Puntos de venta. No expone DNI ni teléfonos.

**Vista administrador** — protegida por clave. Suma las pestañas de Cobertura,
Cargar semana, Pre-carga ZOHO, Directorio y Exportar mes.
...
En Netlify: Project configuration > Environment variables. Scope: Functions.
Despues de agregarlas hay que redesplegar para que tomen efecto.

| Variable | Obligatoria | Para que sirve |
|---|---|---|
| `CLAVE_ADMIN` | si | Clave de acceso al lado administrador |
| `SHEET_TELEFONOS` | no | ID del Google Sheet de telefonos, para leerlo solo |
| `GID_TELEFONOS` | no | Pestana del sheet. Por defecto `0` |

### Telefonos automaticos

Con `SHEET_TELEFONOS` configurada, el tablero lee la hoja de Google por su
cuenta y cruza por `SALESCLERKNAME`. No hay que subir nada.

Los telefonos solo se entregan a quien tenga la clave de administrador.

Requisito: la hoja debe estar compartida como **cualquiera con el enlace puede
ver**. El servidor la lee, no el navegador, asi que la URL no queda expuesta en
el sitio. Aun asi, tenlo presente: quien tenga esa URL puede ver la hoja
completa.

El resultado se guarda en cache 6 horas. Desde la pestana Directorio hay un
enlace para forzar la relectura cuando acabas de cambiar algo.

Si prefieres no compartir la hoja, deja `SHEET_TELEFONOS` sin configurar y usa
el boton **Cargar base de telefonos** de la pestana Directorio.
